...
export function MenuStar({ profile, size, animated, fillFrame = false }: { profile: CelestialVisualProfile; size: number; animated: boolean; fillFrame?: boolean }) {
  ...
  const uniforms = useDerivedValue(() => ({ size: [size, size], bodyRatio: fillFrame ? 0.48 : profile.bodyRatio, phase: phase.value,
    seed: profile.seed, detail: size >= 60 ? 1 : 0, ...palette }), [size, fillFrame, profile.bodyRatio, profile.seed, palette]);
